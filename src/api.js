...
export const fetchOrders = async () => {
  try {
    ...
    if (json.error_code !== 0) {
      throw new Error(json.error_message || 'Unknown API error');
    }

    const allOrders = json.data?.list || [];
    
    // Filter for our provider and inject service names
    return allOrders.filter(order => order.provider === 'smmviews.net').map(order => {
